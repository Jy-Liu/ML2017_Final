...
class Regressor:
    ...
    def train(self, X_raw, Y_raw):
        ...
        X_raw = np.delete(X_raw, 3, 1)

        X_train, X_valid, Y_train, Y_valid = train_test_split(X_raw, Y_raw, test_size=0.2)

        self.forest = RandomForestRegressor()
        self.forest.fit(X_train, Y_train)

        predictions = self.forest.predict(X_valid)
        mae_val = mean_absolute_error(Y_valid, predictions)
    ...
    def predict(self, X_raw, outputs_path):
        ...
        X_raw[:, 0] = [self.city2id[city_name] for city_name in X_raw[:, 0]]
        X_raw = np.delete(X_raw, 3, 1)
        predictions = self.forest.predict(X_raw)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='rf-model.pkl')
